...
Page({
  ...
  toCollect: function () {
    app.tabCollect('fang_id', this.data.fang_id, '3',this)
  },
  ...
  getDetail: function () {
    app.ajax('', {
      ...
      // 存储浏览记录
      app.saveBrowseHistory(2, res.data.data.fang_id);
      // 获取地图图片
  ...
  orderSureBtn: function () {
    console.log(this.data.estate_id)
    app.orderHouse(this.data.orderName, this.data.orderPhone, this.data.estate_id, 2, this.data.detailData.region_id,(res) => {
      this.closeApply();
      this.setData({
        orderName: '',
        orderPhone: '',
  ...
  canOrder: function () {
    console.log(!!this.data.orderPhone)
