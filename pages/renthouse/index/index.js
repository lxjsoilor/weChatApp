...
Page({
  ...
  getFilter: function () {
    // app.getFilter((res) => {
    //   this.setData({
    //     priceList: res.data.data.rent_price,
    //     typeList: res.data.data.house_style
    //   })
    // });
    if (!!app.globalData.filterData) {
      this.setData({
        priceList: app.globalData.filterData.data.rent_price,
        typeList: app.globalData.filterData.data.house_style
      })
    } else {
      app.getFilter((res) => {
        this.setData({
  ...
  selectType: function (e) {
    ...
    this.setData({
      modelid: e.currentTarget.dataset.key,
      page: 0,
      loadMore: 1,
      dataList: [],
      tabText: tempArr
  ...
  getList(page) {
    ...
    app.getList(page, {
      ...
      fang_model: this.data.modelid == 0 ? '' : this.data.modelid,
      rent_price: this.data.priceid == 1 ? '' : this.data.priceid,
      page:page,
      search_estate: this.data.search_estate
    }, (res) => {
      if (res.data.status == 1) {
        // 手动增加房源id
        res.data.data.forEach((val)=>{
          val.fanId = val.fang_key.substring(0, val.fang_key.indexOf("!!"));
        // 去重复地区
  ...
  onLoad: function (options) {
    if(!!options.estate_id){
      this.setData({
        search_estate:options.estate_id
      })
    }
  },
